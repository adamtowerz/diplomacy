...
async function getSigningSecret(): Promise<string> {
  ...
  const secret = await secretsManager.getSecret("deftly-jwt-signing-secret");
...
async function generate({ userId, oid }: { userId: string; oid: string }): Promise<string> {
  ...
  const jwt = JWTLib.sign({ sub: userId, oid }, key, { expiresIn: "3d", issuer: ISSUER, audience: AUDIENCE });
